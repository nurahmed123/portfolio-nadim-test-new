
import pimg1 from '../pic/hexacopter.jpg'
// import pimg5 from '../pic/pimg8.jpg'
// import pimg7 from '../pic/id7.png'
// import pimg8 from '../pic/id8.jpg'
import pimg10 from '../pic/Road-Link.png'
import pimg11 from '../pic/soccer.png'
// import pimg12 from '../pic/GLASS MODEL.jpg'
// import pimg13 from '../pic/PCB.png'
// import pimg14 from '../pic/RICSHAW.jpg'
// import pimg15 from '../pic/QUADCOPTER.jpg'
// import pimg16 from '../pic/GLASS.jpg'
// import pimg17 from '../pic/Box.png'
// import pimg18 from '../pic/INDROUSTRIAL AUTOMATION.jpg'
// import pimg19 from '../pic/PLANE CRASH.jpg' 
// import pimg20 from '../pic/2D DRAW PEB.jpg'
// import pimg21 from '../pic/ESPCAM.jpg'
const Portfolio_data = [
  {
    id: 1,
    category: "Safety",
    totalLike: "5.6k",
    title: "Road LINK",
    description: "The Road LINK is a Bangladeshi innovation designed to revolutionize road safety. This intelligent system utilizes real-time data analysis and proactive hazard identification to prevent accidents, saving lives on roads worldwide.",
    image: pimg10,
  },
  {
    id: 2,
    category: "Drone",
    totalLike: "3.2k",
    title: "Hexa Copter",
    description: "I built my own hexacopter from the ground up, fine-tuning everything from the motors to the flight controller for smooth, stable performance. Whether for aerial photography, mapping, or autonomous flight, this project was all about pushing the limits of what I could create.",
    image: pimg1,
  },
  {
    id: 3,
    category: "Robotic",
    totalLike: "1.7k",
    title: "ROBO Soccer",
    description: "Robo soccer is a team-based competition where robots play soccer against each other. It's a complex task that involves robots working together to achieve goals.",
    image: pimg11,
  },
  // {
  //   id: 4,
  //   category: "Robotic",
  //   totalLike: "630",
  //   title: "Self-Driving Car",
  //   description: "Designed a self-driving car prototype with advanced sensors, AI algorithms, and safety features. The vehicle is capable of autonomous navigation, obstacle avoidance, and real-time decision-making for safe and efficient transportation to partcpate in the WRO Future Engineering Challenge.",
  //   image: pimg7,
  // },
  // {
  //   id: 5,
  //   category: "Robotic",
  //   totalLike: "470",
  //   title: "Design a robot for IRO Robot in movie Category",
  //   image: pimg5,
  // },
  // {
  //   id: 6,
  //   category: "Robotic",
  //   totalLike: "380",
  //   title: "Line Follower Robot",
  //   description: "Make a line follower robot and implemant PID controller for better performance.",
  //   image: pimg8,
  // },
  // {
  //   id: 7,
  //   category: "3D Design",
  //   totalLike: "690",
  //   title: "Smart Glass",
  //   description: "I designed a smart glass in Drishti Technology Ltd during my internship period.",
  //   image: pimg12,
  // },
  // {
  //   id: 8,
  //   category: "PCB",
  //   totalLike: "690",
  //   title: "Design a Professional PCB",
  //   description: "Design a professional PCB.",
  //   image: pimg13,
  // },
  // {
  //   id: 9,
  //   category: "3D Design",
  //   totalLike: "690",
  //   title: "Ricshaw Model",
  //   description: "Design a Ricshaw model.",
  //   image: pimg14,
  // },
  // {
  //   id: 10,
  //   category: "3D Design",
  //   totalLike: "690",
  //   title: "Quadcopter",
  //   description: "Design a Quadcopter model.",
  //   image: pimg15,
  // },
  // {
  //   id: 11,
  //   category: "Prototype",
  //   totalLike: "690",
  //   title: "Smart Glass",
  //   description: "Build a smart glass prototype.",
  //   image: pimg16,
  // },
  // {
  //   id: 12,
  //   category: "Product Design",
  //   totalLike: "690",
  //   title: "Industrial Automation Device Box",
  //   description: "Design a device box for ongshak inrustrial automation project.",
  //   image: pimg17,
  // },
  // {
  //   id: 13,
  //   category: "Product Design",
  //   totalLike: "690",
  //   title: "Industrial Automation Device Box",
  //   description: "build a device for steel tech inrustrial.",
  //   image: pimg18,
  // },
  // {
  //   id: 14,
  //   category: "AVIATION",
  //   totalLike: "690",
  //   title: "Plane",
  //   description: "Design print and build a plane for my hoby project.",
  //   image: pimg19,
  // },
  // {
  //   id: 15,
  //   category: "PCB",
  //   totalLike: "690",
  //   title: "2D Drawing PCB",
  //   description: "Design a 2D drawing PCB using 3D printer.",
  //   image: pimg20,
  // },
  // {
  //   id: 16,
  //   category: "Prototype",
  //   totalLike: "690",
  //   title: "Micro level soldaring",
  //   description: "in this picture i am doing micro level soldaring for my project using ES32 Cam",
  //   image: pimg21,
  // }
]

export default Portfolio_data
